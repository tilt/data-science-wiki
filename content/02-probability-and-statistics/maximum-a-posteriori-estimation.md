...
It is [maximum likelihood](maximum-likelihood.md) plus a prior. In many ML objectives, the log-prior acts like [regularization](../03-classical-machine-learning/regularization.md): a [Gaussian](common-distributions.md#normal-and-gaussian-distributions) prior on weights yields an $L_2$ penalty.

## Worked scenario

A coin is tossed 10 times and lands heads 7 times. With a flat [$\mathrm{Beta}(1,1)$](common-distributions.md#beta-distribution) prior, the posterior mode is the observed head rate, $0.7$, because the prior contributes no pull toward either side. With a symmetric $\mathrm{Beta}(2,2)$ prior, the prior behaves like one prior head and one prior tail for the posterior mode, giving $(7+1)/(10+2)=0.6667$.
...
>
> [← Maximum Likelihood](maximum-likelihood.md) [Bayesian Statistics →](bayesian-statistics.md)

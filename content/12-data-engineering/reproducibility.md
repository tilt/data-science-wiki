...
[Data-lineage](data-lineage.md) records which job produced which dataset; reproducibility records enough detail to rebuild it. [Data-contracts](data-contracts.md) pin the expected schema and semantics, while [data-quality](data-quality.md) records whether the rebuilt output still satisfies the contract. For ML, link this metadata to [dataset-versioning](../13-ml-engineering-and-mlops/dataset-versioning.md) so model metrics can be traced to a dataset snapshot.

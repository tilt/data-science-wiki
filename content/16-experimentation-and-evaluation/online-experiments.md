...
Interference breaks independent-unit assumptions when one user's treatment changes another user's experience. Peeking creates false positives unless the sequential rule is planned. Canary rollout is not the same as a randomized experiment: [canary deployment](../13-ml-engineering-and-mlops/canary-deployment.md) protects reliability, while randomization supports causal measurement.

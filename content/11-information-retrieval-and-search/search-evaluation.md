...
Judgment pools are incomplete: an unjudged document may be relevant. Query logs are biased toward what the old system could answer. Offline improvements also may not improve user behavior, so mature systems connect offline tests to [online experiments](../16-experimentation-and-evaluation/online-experiments.md), human review, and risk-weighted slices.

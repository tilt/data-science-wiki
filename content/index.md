---
title: Data Science, Machine Learning & AI Wiki
slug: index
description: Central entry point for the personal data science, machine learning, and AI wiki.
area: home-and-navigation
topics:
  - "navigation"
  - "taxonomy"
  - "learning-paths"
level: foundational
status: review
page_type: area-index
aliases:
  - "Home"
  - "Start Here"
prerequisites: []
related:
  - "00-home-and-navigation"
  - "01-mathematical-foundations"
  - "02-probability-and-statistics"
  - "03-classical-machine-learning"
  - "04-recommendation-systems"
  - "05-time-series-and-forecasting"
  - "06-deep-learning"
  - "07-reinforcement-learning"
  - "08-natural-language-processing"
  - "09-computer-vision"
  - "10-video-understanding"
  - "11-generative-ai"
  - "12-information-retrieval-and-search"
  - "13-data-engineering"
  - "14-ml-engineering-and-mlops"
  - "15-cloud-and-distributed-systems"
  - "16-software-engineering"
  - "17-experimentation-and-evaluation"
  - "18-responsible-ai-safety-and-governance"
  - "19-domain-applications"
  - "20-history-of-ai-and-machine-learning"
  - "21-references-and-glossary"
historical_context: false
last_reviewed: 2026-07-17
---

# Data Science, Machine Learning & AI Wiki

## Start routes

- [Browse by subject](00-home-and-navigation/index.md)
- [Browse by learning path](00-home-and-navigation/learning-paths.md)
- [Browse by technical answer pattern](00-home-and-navigation/technical-answer-patterns.md)
- [Browse by mathematical prerequisite](01-mathematical-foundations/index.md)
- [Browse by production lifecycle](14-ml-engineering-and-mlops/index.md)
- Browse by tags through Quartz tag pages
- Browse through the graph using the Quartz graph panel

## Major areas

- [Home and Navigation](00-home-and-navigation/index.md)
- [Mathematical Foundations](01-mathematical-foundations/index.md)
- [Probability and Statistics](02-probability-and-statistics/index.md)
- [Classical Machine Learning](03-classical-machine-learning/index.md)
- [Recommendation Systems and Personalization](04-recommendation-systems/index.md)
- [Time-Series Forecasting](05-time-series-and-forecasting/index.md)
- [Deep Learning](06-deep-learning/index.md)
- [Reinforcement Learning](07-reinforcement-learning/index.md)
- [Natural Language Processing](08-natural-language-processing/index.md)
- [Computer Vision](09-computer-vision/index.md)
- [Video Understanding](10-video-understanding/index.md)
- [Generative AI and Agentic Systems](11-generative-ai/index.md)
- [Information Retrieval and Search](12-information-retrieval-and-search/index.md)
- [Data Engineering](13-data-engineering/index.md)
- [ML Engineering and MLOps](14-ml-engineering-and-mlops/index.md)
- [Cloud and Distributed Systems](15-cloud-and-distributed-systems/index.md)
- [Software Engineering](16-software-engineering/index.md)
- [Experimentation and Evaluation](17-experimentation-and-evaluation/index.md)
- [Responsible AI, Safety, and Governance](18-responsible-ai-safety-and-governance/index.md)
- [Domain Applications](19-domain-applications/index.md)
- [History of AI and Machine Learning](20-history-of-ai-and-machine-learning/index.md)
- [References and Glossary](21-references-and-glossary/index.md)

## Authoring policy

Canonical content is Markdown in this `content/` directory. Pages use portable Markdown links, YAML front matter, `$...$` and `$$...$$` math, fenced Mermaid diagrams, and source-controlled media assets.

...
Content-based image retrieval finds images by visual similarity rather than only text metadata. The core dependency is [feature extraction](feature-extraction.md): the retrieval system can only rank by what the descriptor or embedding preserves. It connects directly to vector search in [vector indexes](../11-information-retrieval-and-search/vector-indexes.md).

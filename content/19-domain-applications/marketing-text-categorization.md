...
The core method is [text classification](../08-natural-language-processing/text-classification.md), often with sparse n-grams, embeddings, or encoder models. [Text preprocessing](../08-natural-language-processing/text-preprocessing.md) must preserve tokens that matter commercially, such as discount numbers, currency, expiration dates, and opt-out phrases. Evaluation should report macro-F1 by label, precision for regulated classes, calibration for human review thresholds, and slice performance by locale, channel, and campaign type. Labels are often skewed, so [class imbalance](../03-classical-machine-learning/class-imbalance.md) and [calibration](../03-classical-machine-learning/calibration.md) should be explicit.
...
Copy changes quickly around launches and seasonal campaigns, so stale examples cause drift. Ambiguous text can belong to multiple [topic classification](../08-natural-language-processing/topic-classification.md) labels, such as "webinar invite" being both nurture and event. Do not train on downstream engagement outcomes as if they were taxonomy labels; that changes the task from categorization to response prediction.
